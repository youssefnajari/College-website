...
$r = mysqli_query($db, "SELECT Name, LastName, StID FROM Student WHERE ClID='C1'");
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($d) || empty($da) || empty($id)) {
        ...
        $r1 = mysqli_query($db, "INSERT INTO session (TeaID,ClID, Start, Duration) VALUES ('$id','C1', str_to_date('$da', '%Y-%m-%d %H:%i:%s'), '$d')");

        if (!$r1) {
            die("Query failed: " . mysqli_error($db));
        }

        $r2 = mysqli_query($db, "SELECT Name, LastName, StID FROM Student WHERE ClID='C1'");
